...
const CHART_MONTHLY = [
  ...
  { month: "Feb", revenue: 48500, bookings: 3.2 },
  { month: "Mar", revenue: 54200, bookings: 3.7 },
  { month: "Apr", revenue: 61800, bookings: 4.3 },
  { month: "May", revenue: 58400, bookings: 4.0 },
  { month: "Jun", revenue: 72100, bookings: 5.1 },
  { month: "Jul", revenue: 84300, bookings: 5.9 },
  { month: "Aug", revenue: 79800, bookings: 5.5 },
  { month: "Sep", revenue: 68200, bookings: 4.6 },
  { month: "Oct", revenue: 63400, bookings: 4.3 },
  { month: "Nov", revenue: 57900, bookings: 3.8 },
  { month: "Dec", revenue: 71200, bookings: 4.9 },
];
...
export default function DashboardPage() {
  ...
  myProps.forEach((p) => {
    const t = p.type.charAt(0).toUpperCase() + p.type.slice(1) + "s";
    typeCounts[t] = (typeCounts[t] || 0) + 1;
  });

  const dynamicChartTypes = Object.entries(typeCounts).map(([name, count]) => ({
    name,
    value: Math.round((count / myProps.length) * 100),
  }));

  const chartTypesToRender = dynamicChartTypes.length > 0 ? dynamicChartTypes : [
    { name: "No Listings", value: 100 }
  ];

  // Dynamic income vs expenses monthly data
  const baseRevenue = myProps.reduce((s, p) => s + p.rent, 0);
  const dashData = CHART_MONTHLY.slice(-6).map((d) => {
    // Scale revenue based on user properties if they exist, otherwise show general demo values
    const monthlyIncome = baseRevenue > 0
      ? Math.round(baseRevenue * d.bookings)
      : Math.round(d.revenue / 12);
    const monthlyExpenses = Math.round(monthlyIncome * 0.22);
    return {
      month: d.month,
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {myProps.length === 0 ? (
          <div className="bg-white rounded-2xl p-16 border border-gray-100 shadow-sm text-center mb-8">
            <Building2 size={64} className="text-slate-200 mx-auto mb-4" />
            <h2 className="text-xl font-bold text-slate-700 mb-2">No Dashboard Data Available</h2>
            <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
              Create a rental property listing to activate your earnings dashboard, view analytics, and track monthly guest reviews.
            </p>
            <button
              onClick={() => router.push("/add-property")}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl transition-colors border-none cursor-pointer"
            >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8 text-left">
              <div className="lg:col-span-2 bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-between">
                <div className="mb-5">
                  ...
                  <p className="text-xs text-slate-400">Based on active listing rents</p>
                </div>
